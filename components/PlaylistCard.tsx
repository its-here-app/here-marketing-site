const PlaylistCard = ({ className = "", index = 0, playlist = null }) => {
  if (playlist) {
    return (
      <div
        key={index}
        className={`w-[15rem] h-[15rem] md:w-[20rem] md:h-[20rem] xl:w-[28rem] xl:h-[28rem] bg-gray-100 rounded-2xl mr-[clamp(0.5rem,2vw,1.25rem)] transition-all duration-1200 ease-in-out ${className}`}
      >
